...
namespace FileMonitorController
{
    public partial class FormMain : Form
    {
        ...
        private void PushFileEntryEvent(ProcessEntry whichProcess, List<MessageEntry> fileEntry)
        {
        ...
        private void Timer_UpdateMonitorLog_Tick(object sender, EventArgs e)
        {
            ...
            foreach (int index in selectedIndices)
            {
                ...
                foreach (var processId in IpcInterface.Entries.Keys)
                {
                    if (processId == associatedProcess.Id)
                    {
                        PushFileEntryEvent(associatedProcess, IpcInterface.Entries[processId]);
                    }
                }
            }
